...
When using `orchestrator: k3s`, images are always imported via `k3s ctr images import` (containerd) regardless of the build engine. You can build with Docker or Podman locally, and Dockflow handles the transfer to containerd on the k3s node.

```
# With Docker + k3s:
docker save myapp:1.0 | ssh node "sudo k3s ctr images import -"

# With Podman + k3s:
podman save myapp:1.0 | ssh node "sudo k3s ctr images import -"
```
