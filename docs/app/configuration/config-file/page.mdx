...
Create `.dockflow/config.yml` to customize behavior.
...
options:
  image_auto_tag: true          # Auto-append -<env>:<version> to images (default: true)
...
# Deploy lock
lock:
  stale_threshold_minutes: 30 # Lock expires after N minutes (default: 30)
...
```yaml
options:
  image_auto_tag: true   # Images become my-app-production:1.0.0
  image_auto_tag: false  # Images keep original names as-is
```
...
Lock files are stored in `/var/lib/dockflow/locks/` and automatically expire after the configured stale threshold (default: 30 minutes).

### `stale_threshold_minutes`
**Default:** `30`

Minutes after which a deployment lock is considered stale and can be overridden. Accepts values from 1 to 1440 (24 hours).

```yaml
lock:
  stale_threshold_minutes: 60   # Lock expires after 60 minutes instead of 30
```
...
2024-01-17T09:15:00Z | DEPLOYED | 1.0.2 | user@ci-runner | with warnings
```
