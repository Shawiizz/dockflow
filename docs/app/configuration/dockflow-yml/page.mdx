...
When you run any `dockflow` command, it traverses up from the current directory looking for `dockflow.yml` or `.dockflow/`. If `dockflow.yml` is found first, rootless mode is activated.
...
- **Accessories**: a separate `accessories.yml` file is not supported in rootless mode — define accessories inline under the `accessories` key in `dockflow.yml`.
