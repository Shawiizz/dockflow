...
Dockflow configures Swarm updates with these defaults:

| Setting | Value | Description |
|---------|-------|-------------|
| `parallelism` | 1 | Update one container at a time |
| `delay` | 10s | Wait between container updates |
| `failure_action` | rollback | Revert on failure |
| `monitor` | 30s | Monitor period after update |
| `order` | start-first | Start new container before stopping old |

These settings ensure zero-downtime deployments for services with at least 2 replicas.
