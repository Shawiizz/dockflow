...
No external registry is required. For multi-node clusters, images are streamed to each node via SSH by default. You can optionally configure a [Docker registry](/configuration/registry) for faster transfers on larger clusters.
...
By default, Dockflow streams images to all nodes via SSH. For larger clusters where SSH streaming becomes slow, you can configure a registry so nodes pull images independently:
...
See [Registry Configuration](/configuration/registry) for setup instructions and [Build Strategy](/configuration/build-strategy) for a comparison of transfer methods.
