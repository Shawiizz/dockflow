...
<Tabs items={['CLI', 'Tag (CI/CD)', 'Branch (CI/CD)']}>
  ...
  <Tabs.Tab>
    Push a git tag to trigger a deployment:
    ...
    A tag without a suffix (e.g., `1.0.0`) deploys to `production`. A tag ending with `-<env>` (e.g., `1.0.0-staging`) deploys to that environment.
  </Tabs.Tab>
  <Tabs.Tab>
    Push to a branch to deploy the latest commit:
