...
Enabling the proxy does **not** disable nginx template deployment — the two run independently.
Traefik is deployed as a Swarm service publishing ports 80 and 443, while any file in
`.dockflow/templates/nginx/` is still installed into `/etc/nginx/sites-enabled/` and nginx is
reloaded. A host nginx listening on port 80 and Traefik publishing port 80 cannot both bind it.

Pick one: either remove your nginx templates and route through Traefik labels, or leave
`proxy.enabled: false` and keep managing vhosts yourself.
