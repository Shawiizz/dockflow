...
This is the simplest strategy and works for both single-node deployments and multi-node clusters. For larger clusters with many nodes or heavy images, consider using a [registry](/en/configuration/registry) instead.
...
See [Docker Registry](/en/configuration/registry) for full configuration (GHCR, GitLab, Docker Hub, self-hosted).
...
**Next:** [Deployment](/en/deployment) — understand what happens when you deploy.
