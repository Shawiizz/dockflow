...
Files placed in `.dockflow/templates/nginx/` are treated as nginx configurations and automatically deployed to `/etc/nginx/sites-enabled/` on the server after each successful deployment. The filename is used verbatim — name the file exactly as nginx should see it.
