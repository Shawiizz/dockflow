# Hooks

import { Callout, FileTree } from 'nextra/components'

Hooks allow you to run custom scripts at specific points during deployment.

## Available hooks

| Hook | Location | Timing |
|------|----------|--------|
| `pre-build` | Local (CI runner) | Before Docker images are built |
| `post-build` | Local (CI runner) | After Docker images are built |
| `pre-upload` | Server | Before files from `uploads` entries are copied to the server |
| `post-upload` | Server | After files are uploaded, before stack deployment |
| `pre-deploy` | Server | After file uploads, before stack deployment |
| `post-deploy` | Server | After successful deployment and health checks |

<Callout type="info">
  When [`remote_build: true`](/en/configuration/build-strategy) is set, `pre-build` and `post-build` hooks run on the server instead of locally — where the build is actually happening.
</Callout>

<Callout type="info">
  Local hooks run through bash. On Windows, dockflow automatically uses Git Bash when installed (the System32 WSL `bash.exe` stub is never used) — install [Git for Windows](https://git-scm.com/download/win) to run local hooks.
</Callout>

## File-based hooks

Create shell scripts in `.dockflow/hooks/`:

<FileTree>
  <FileTree.Folder name=".dockflow" defaultOpen>
    <FileTree.Folder name="hooks" defaultOpen>
      <FileTree.File name="pre-build.sh" />
      <FileTree.File name="post-build.sh" />
      <FileTree.File name="pre-upload.sh" />
      <FileTree.File name="post-upload.sh" />
      <FileTree.File name="pre-deploy.sh" />
      <FileTree.File name="post-deploy.sh" />
    </FileTree.Folder>
  </FileTree.Folder>
</FileTree>

All hooks are optional. If a hook file doesn't exist, it is skipped.

## Inline hooks

For simple commands, define them directly in `.dockflow/config.yml` instead of creating a separate script:

```yaml
# .dockflow/config.yml
hooks:
  post-deploy: systemctl reload nginx

  pre-build:
    - echo "Building {{ config.project_name }} {{ version }}"
    - docker pull base-image:latest
```

When both a file and inline commands are defined for the same phase, the file runs first, then the inline commands.

## Templating

Hooks are rendered with [Nunjucks](https://mozilla.github.io/nunjucks/) before execution. You can use any template variable:

```bash
#!/bin/bash
echo "Deploying {{ config.project_name }} version {{ version }} to {{ env }}"

# Access server environment variables
DATABASE_URL="{{ current.env.database_url }}"
```

## Examples

### Pre-build: Generate assets

```bash
# .dockflow/hooks/pre-build.sh
#!/bin/bash
npm run build:assets
```

### Post-build: Security scan

```bash
# .dockflow/hooks/post-build.sh
#!/bin/bash
docker scan {{ project_name }}:{{ version }} || true
```

### Pre-upload: Stop a service before files arrive

```bash
# .dockflow/hooks/pre-upload.sh
#!/bin/bash
systemctl stop myapp || true
```

### Post-upload: Run a migration on uploaded files

```bash
# .dockflow/hooks/post-upload.sh
#!/bin/bash
cd /var/www/myapp && php artisan migrate --force
```

### Pre-deploy: Database backup

```bash
# .dockflow/hooks/pre-deploy.sh
#!/bin/bash
pg_dump $DATABASE_URL > /backups/pre-deploy-{{ version }}.sql
```

### Post-deploy: Reload a systemd service

```yaml
# .dockflow/config.yml
hooks:
  post-deploy:
    - systemctl daemon-reload
    - systemctl restart rclone-s3.service
```

### Post-deploy: Notify team

```bash
# .dockflow/hooks/post-deploy.sh
#!/bin/bash
curl -X POST "https://hooks.slack.com/services/xxx" \
  -H "Content-Type: application/json" \
  -d '{"text":"Deployed {{ project_name }} {{ version }} to {{ env }}"}'
```

## Configuration

```yaml
# .dockflow/config.yml
hooks:
  enabled: true    # Enable/disable all hooks (default: true)
  timeout: 300     # Timeout per hook in seconds (default: 300)
  fatal: false     # Abort deploy on hook failure (default: false)

  pre-build: echo "starting build"
  post-deploy:
    - systemctl daemon-reload
    - systemctl restart myservice
```

| Field | Type | Description | Default |
|-------|------|-------------|---------|
| `enabled` | boolean | Enable or disable all hooks | `true` |
| `timeout` | number | Max execution time per hook, in seconds | `300` |
| `fatal` | boolean \| string[] | Abort the deploy when a hook exits with a non-zero code. A boolean covers every hook; a list of phase names makes only those fatal | `false` |
| `pre-build` | string \| string[] | Inline command(s) for the pre-build phase | — |
| `post-build` | string \| string[] | Inline command(s) for the post-build phase | — |
| `pre-upload` | string \| string[] | Inline command(s) to run on the server before files are uploaded | — |
| `post-upload` | string \| string[] | Inline command(s) to run on the server after files are uploaded | — |
| `pre-deploy` | string \| string[] | Inline command(s) for the pre-deploy phase | — |
| `post-deploy` | string \| string[] | Inline command(s) for the post-deploy phase | — |

By default, hook failures print a warning and let the deploy continue. Set `fatal: true` to treat a non-zero exit code as a deploy error — useful for hooks that run critical operations like database migrations.

`fatal` also accepts a list of phases, so one critical hook can abort the deploy without turning every other hook into a deploy blocker:

```yaml
# .dockflow/config.yml
hooks:
  fatal: [post-upload]           # only this phase aborts
  post-upload: nginx -t && nginx -s reload
  post-deploy: curl -s https://hooks.example.com/notify || true
```

Here an invalid nginx configuration fails the deploy — and because uploads are rolled back when a deploy fails, the previous configuration is restored — while a webhook that cannot be reached stays a warning.

<Callout type="warning">
  With `fatal: false` (the default), a failing hook does **not** abort the deploy. If your hook runs a critical operation (e.g. a migration), make that phase fatal.
</Callout>
