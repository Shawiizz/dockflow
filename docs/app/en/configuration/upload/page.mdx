...
<Steps>
### Permission check
Before anything is built, dockflow verifies that the deploy user has write access to all upload destinations on every node. If a destination doesn't exist yet, its nearest existing parent directory is checked instead. The deploy fails immediately with a clear fix command if any permission is missing.

### Build
Once permissions are confirmed, Docker images are built and distributed to the nodes.

### `pre-upload` hook
Runs before any file is transferred — the place for anything the uploads, or a plugin's `post-upload` entries, depend on.

### Transfer to all nodes
Files are transferred to the manager **and all worker nodes** in parallel, so bind mounts work regardless of which node runs the container. If the deployment fails at any point after this step, every uploaded file is restored to its previous version.

### `post-upload` and `pre-deploy` hooks
Both run after the transfer, so they can use the uploaded files directly — `systemctl daemon-reload` after uploading a systemd unit, for instance.
...
Files inside `.dockflow/` are automatically rendered through [Nunjucks](https://mozilla.github.io/nunjucks/) before being uploaded. This means you can use any template variable in your uploaded files:
...
→ Run once on each failing server as root:
  mkdir -p '/etc/systemd/system' && chown dockflow: '/etc/systemd/system'
```

The command targets the **directory**, not the file: uploads write with `cat > path`, which
needs write access on the directory that contains the file. For a directory upload, the
destination itself is the target:
...
- [Hooks](/en/configuration/hooks) — use `pre-deploy` to reload services after upload (e.g. `systemctl daemon-reload`)
- [Templates](/en/configuration/templates) — full reference for available template variables
