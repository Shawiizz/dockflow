...
| `hooks` | object | Commands and scripts to run at each deploy phase ([reference](/en/configuration/hooks)) | — |
| `plugins` | array | Plugins contributing uploads and hooks ([reference](/en/configuration/plugins)) | — |
...
| `uploads` | array | Files or directories to transfer to the servers ([reference](/en/configuration/upload)) | — |
| `no_services` | boolean | Upload-only project, with no Docker services | `false` |
...
See [Servers & Connections](/en/configuration/servers) and [Project Configuration](/en/configuration/config-file) for full documentation of each field.
...
- **Accessories**: place `accessories.yml` at the project root alongside `dockflow.yml`. If not found there, Dockflow falls back to `.dockflow/docker/accessories.yml`.
...
- [Project Configuration](/en/configuration/config-file) — all `config.yml` fields in detail
- [Servers & Connections](/en/configuration/servers) — all `servers.yml` fields in detail
- [Docker Compose](/en/configuration/docker-compose) — compose file structure and Dockflow-specific behavior
