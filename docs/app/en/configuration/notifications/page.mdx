...
`config.yml` is rendered by Nunjucks before parsing. `{{ current.env.* }}` references server-level env vars defined in `servers.yml`. In CI, those values come from your CI secrets rather than being hardcoded — see [Servers & Connections](/en/configuration/servers).
  </Tabs.Tab>
</Tabs>
