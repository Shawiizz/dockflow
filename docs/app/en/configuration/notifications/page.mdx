...
`config.yml` (or `dockflow.yml`) is rendered through Nunjucks before being parsed, so you can reference server-level env vars:
...
`config.yml` (or `dockflow.yml`) is rendered by Nunjucks before parsing. `{{ current.env.* }}` references server-level env vars defined in `servers.yml`. In CI, those values come from your CI secrets rather than being hardcoded — see [Servers & Connections](/en/configuration/servers).
  </Tabs.Tab>
</Tabs>
...
- [Hooks](/en/configuration/hooks) — run arbitrary scripts at build and deploy time, including custom notification logic
