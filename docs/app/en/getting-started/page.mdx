...
- **`config.yml`** — Project name, build strategy, health checks ([reference](/en/configuration))
- **`servers.yml`** — Servers and environment variables ([reference](/en/configuration/servers))
- **`docker/`** — Docker Compose stack, accessories, and Dockerfile
- **`hooks/`** — Pre/post build and deploy hook scripts ([reference](/en/configuration#hooks))
