...
<Callout type="info">
  You can point any AI assistant to `https://your-domain/llms.txt` to give it context about Dockflow.
</Callout>

## MCP Server

The `@dockflow/mcp` package provides a [Model Context Protocol](https://modelcontextprotocol.io/) server that exposes Dockflow documentation as tools for AI assistants.

### Installation

<Tabs items={['Claude Code', 'Claude Desktop', 'Other MCP clients']}>
  <Tabs.Tab>
    ```bash
    claude mcp add dockflow -- npx -y @dockflow/mcp
    ```
  </Tabs.Tab>
  <Tabs.Tab>
...
| `get_config_reference` | Get configuration reference (servers.yml, config.yml, etc.) |
| `get_cli_commands` | Get the full CLI command reference |
