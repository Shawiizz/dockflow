...
prepare_custom_templates() {
	...
	command -v yq &>/dev/null || {
		log_warning "yq not found, skipping custom templates"
		return 0
	}

	local count
	count=$(yq '.templates | length // 0' "$CONFIG_FILE" 2>/dev/null || echo "0")
	...
	for i in $(seq 0 $((count - 1))); do
		local dest
		dest=$(yq -r ".templates[$i].dest // .templates[$i]" "$CONFIG_FILE" 2>/dev/null)
		local src
		src=$(yq -r ".templates[$i].src // .templates[$i]" "$CONFIG_FILE" 2>/dev/null)
